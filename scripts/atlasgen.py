...
def agent_entrypoint(flavour: str) -> str:
    ...
    lines = [
        ...
        "",
        "```bash",
        "python scripts/verify.py            # every gate below, one verdict each; exit 0 only if all PASS",
        *[" ".join(g["argv"]) for g in (atlas().get("verification_policy") or {}).get("done_set") or []],
        "```",
        "",
