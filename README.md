...
- **Before routing:** an agent reads 1,739 tokens. The other 156 documents (479 KiB) load only when a route names one.
- **Coverage:** all 324 language × check pairs answer — 133 with a command, 191 with a declared *no tool*, 0 silently.
- **Mistakes caught:** 206 kinds are planted in the tests, and each must be refused.
...
- **contract version:** 3.19.0 — `VERSION`, asserted at a declared line in 6 other files
