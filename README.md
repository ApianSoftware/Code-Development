...
**36** languages · **53** extensions · **64** gates · **8** runtimes · **44** failure shapes · **38** invariants · **48** instruments · **1** dependency
...
- **Before routing:** an agent reads 1,747 tokens. The other 156 documents (478 KiB) load only when a route names one.
- **Coverage:** all 324 language × check pairs answer — 133 with a command, 191 with a declared *no tool*, 0 silently.
- **Mistakes caught:** 203 kinds are planted in the tests, and each must be refused.
...
- **contract version:** 3.18.0 — `VERSION`, asserted at a declared line in 6 other files
...
- **instruments:** 48 — `atlas.yaml/instruments`, each naming its own limits
- **verification gate classes:** 8 — `atlas.yaml/verification_policy/profiles`
- **task profiles:** 14 — `atlas.yaml/task_profiles`
- **python files in the harness:** 48 — `scripts/*.py`, all linted by ruff
